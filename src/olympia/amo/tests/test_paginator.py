...
from olympia.amo.paginator import (
    EmptyPage, ESPaginator, InvalidPage, PageNotAnInteger, Paginator)
from olympia.amo.templatetags.jinja_helpers import PaginatorRenderer
...
def assert_range(page_number, num_pages, expected):
    p = PaginatorRenderer(mock_pager(page_number, num_pages, 100))
...
def test_dots():
    p = PaginatorRenderer(mock_pager(1, 5, 100))
    assert not p.pager.dotted_upper
    assert not p.pager.dotted_lower

    p = PaginatorRenderer(mock_pager(1, 25, 100))
    assert p.pager.dotted_upper
    assert not p.pager.dotted_lower

    p = PaginatorRenderer(mock_pager(12, 25, 100))
    assert p.pager.dotted_upper
    assert p.pager.dotted_lower

    p = PaginatorRenderer(mock_pager(24, 25, 100))
    assert not p.pager.dotted_upper
    assert p.pager.dotted_lower
