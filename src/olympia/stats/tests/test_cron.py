...
from olympia.stats.models import (
    DownloadCount, GlobalStat, UpdateCount)
...
class TestIndexStats(TestCase):
    ...
    def test_by_date(self, group_mock):
        ...
        download_group = group_mock.call_args[0][0][0]
        assert len(download_group) == 1
        task = download_group.tasks[0].tasks[0]
        assert task.task == 'olympia.stats.tasks.index_download_counts'
        assert task.args == (list(qs), None)

    def test_both_groups_are_called(self, group_mock):
        call_command('index_stats', addons=None, date='2009-06-01')
        assert len(group_mock.call_args[0][0]) == 2

    def test_both_groups_are_called_with_addons_param(self, group_mock):
        call_command('index_stats', addons='4,5,6')
        assert len(group_mock.call_args[0][0]) == 2

    def test_groups_are_filtered(self, group_mock):
        # There are no update counts for this addon, so there should only be
        # one group of tasks.
        call_command('index_stats', addons='5')
        assert len(group_mock.call_args[0][0]) == 1
    ...
    def test_no_addon_or_date(self, group_mock):
        # Indexing the whole world!
        call_command('index_stats', addons=None, date=None)

        # Second group, the downloads.
        ...
        # 9 calls.
        download_counts_group = group_mock.call_args[0][0][0]
        assert len(download_counts_group) == 9

        for task in download_counts_group.tasks[0].tasks:
            assert task.task == 'olympia.stats.tasks.index_download_counts'

        # Second group, the updates.
        # There should be 3 updates, but 2 of them have a date close enough
        # together that they'll be indexed in the same chunk, so we should have
        # 2 calls.
        update_counts_group = group_mock.call_args[0][0][1]
        assert len(update_counts_group) == 2

        for task in update_counts_group.tasks[0].tasks:
            assert task.task == 'olympia.stats.tasks.index_update_counts'
