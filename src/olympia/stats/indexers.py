...
class StatsIndexer(BaseSearchIndexer):
    ...
    def reindex_tasks_group(cls, index_name, addons=None, dates=None):
        """
        Return the list of task groups to execute to index statistics for the
        given index/dates/addons.
        """
        ...
        if not (dates or addons):
            ...
            for start in range(0, num_days, cls.FULL_REINDEX_DAYS_SLICE):
                ...
                data_qs = qs.filter(date__range=date_range)
                if data_qs.exists():
                    index_data_tasks = create_chunked_tasks_signatures(
                        cls.get_indexing_task(), data_qs, cls.CHUNK_SIZE,
                        task_args=(index_name,))
                    if index_data_tasks.tasks:
                        jobs.append(index_data_tasks)
        else:
            data_qs = qs
            if data_qs.exists():
                index_data_tasks = create_chunked_tasks_signatures(
